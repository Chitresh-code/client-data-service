...
## Project Notes (client-data-service)

This is the generic reference doc; the workflow (branches, commits, work items, wiki)
lives in `CONTRIBUTING.md` — follow that too, at all times.

Stack: NestJS, TypeScript, TypeORM + Postgres, class-validator/class-transformer,
`@nestjs/config` (Joi-validated), `@nestjs/throttler`, `@nestjs/terminus`,
`nestjs-pino` for structured JSON logs, JWT auth via `passport-jwt` + `jwks-rsa`
verifying against identity-service's published JWKS (no shared secret — see that
repo's "JWKS & Token Issuance" wiki page).

Already wired in the scaffold (`src/`):
- §1.2 feature-module layout (`src/{domain}/`, one module per bounded context)
- §2.4/2.6 constructor injection throughout; `Public`/`SkipThrottle` via injection
  tokens + `Reflector`, not service-locator lookups
- §3.3 global `AllExceptionsFilter` (`src/common/filters/`)
- §4.1/4.4 `JwtAuthGuard` + `JwtStrategy` wired as a global `APP_GUARD`
  (`src/auth/`), `@Public()` escapes it for `/health/*`
- §4.2 global `ThrottlerModule` limit, `@SkipThrottle()` on health checks
- §4.5 global `ValidationPipe({ whitelist, forbidNonWhitelisted, transform })`
- §7.2 TypeORM migrations (`npm run migration:generate|run|revert`), no
  `synchronize: true` anywhere
- §9.1 liveness/readiness split in `src/health/` (`/health/live`, `/health/ready`)
- §10.1 `enableShutdownHooks()`; §10.2 `ConfigModule` + Joi; §10.3 `nestjs-pino`

Deliberately not yet built — add when the first real need shows up, not before
(YAGNI): §1.5 event-driven decoupling, §5.4 caching, §8.1 response DTOs/serialization
(no domain entities exist yet), §8.4 API versioning, §9.2/9.3 message
patterns/queues. Pull the relevant section back out when a work item needs it.

`jwks-rsa` is pinned to `3.x`, not `4.x` — the `4.x` line depends on the ESM-only
`jose` package, which `ts-jest` can't transform without extra config. `3.x` is CJS
